...
class cycle_gan():
    ...
    def __init__(self,args,sess):
        ...
        self.check_nan = bool(args.check_nan)
    ...
    def build_model(self):
        ...
        def build_XYX_graph():
            ...
            with tf.variable_scope("generator_Y2X") as scope:
                #X2Y_outputs_len = tf.reduce_sum(tf.sign(X2Y_outputs), 1)
        ...
        with tf.variable_scope('generator_loss') as scope:
            ...
            self.generator_X2Y_loss = (self.Y2X_reconstruction_loss + self.X2Y_reconstruction_loss)*2.0 - self.false_Y_sample_score + self.Y2Y_identity_loss
    ...
    def pretrain(self):
        ...
        for X_batch,Y_batch in self.utils.pretrain_generator_data_generator():
            ...
            if self.check_nan:
                ...
                _,_,loss_X2Y,loss_Y2X,tt = self.sess.run(ops,feed_dict=feed_dict)
    ...
    def train(self): 
        ...
        #init loss
        dis_X_loss = 0.0;dis_Y_loss = 0.0;xy_l = 0.0;yx_l = 0.0;xy_r = 0.0;yx_r = 0.0;xx_i = 0.0;yy_i = 0.0;
        ...
        for real_X_batches,real_Y_batches in self.utils.gan_data_generator():
            ...
            #train discriminator
            ops = [self.train_discriminator_X_op,self.train_discriminator_Y_op,self.discriminator_X_loss,self.discriminator_Y_loss]
            if self.check_nan:
                ops.append(self.check_nan_op())
            for i in range(self.discriminator_iterations):
                ...
                if self.check_nan:
                    _,_,loss_X,loss_Y,check_nan_op = self.sess.run(
                        ops,\
                        feed_dict=feed_dict)
                else:
            ...
            if step>=self.pretrain_discriminator_steps:
                ...
                if self.check_nan:
                    ops = [self.train_X2Y_op,\
                           self.train_Y2X_op,\
                           #self.Y2Y_identity_op,\
                           #self.X2X_identity_op,\
                           self.generator_X2Y_loss,\
                           self.generator_Y2X_loss,\
                           self.Y2Y_identity_loss,\
                           self.X2X_identity_loss,\
                           self.X2Y_reconstruction_loss,\
                           self.Y2X_reconstruction_loss,\
                           self.X2Y_test_outputs,\
                           self.check_nan_op()] 
                    _,_,l0,l1,i0,i1,r0,r1,pred,check_nan_op = self.sess.run(
                        ops,\
                        feed_dict=feed_dict)
                else:
                    ops = [self.train_X2Y_op,\
                           ...
                           self.Y2X_reconstruction_loss,\
                           self.X2Y_test_outputs] 
                    _,_,l0,l1,i0,i1,r0,r1,pred = self.sess.run(
                        ops,\
                        feed_dict=feed_dict)
    ...
    def test(self):
        ...
        while(sentence):
            ...
            if self.check_nan:
                ops,check_nan_op = [self.X2Y_test_outputs,self.check_nan_op()]
            else:
                ops = [self.X2Y_test_outputs]
            preds = self.sess.run(ops,feed_dict)
    ...
    def file_test(self):
        ...
        for test_batch in self.utils.test_data_generator():
            ...
            if self.check_nan:
                preds,check_nan_op = self.sess.run(ops,feed_dict)
            else:
                preds = self.sess.run(ops,feed_dict)
            preds = preds[0]
            for pred in preds:
